...
class VirtualProfileManager(context: Context) {
    ...
    /**
     * Generate and save a profile with a specific device preset.
     */
    fun applyPreset(cloneId: String, userId: Int, presetNameOrId: String): VirtualProfileEntity {
        val packageName = "com.android.vending"
        val profile = generator.generateProfileWithPreset(packageName, userId, presetNameOrId)
        val copy = profile.copy(cloneId = cloneId)
        
        val existing = db.dao.getById(cloneId)
        if (existing != null) {
            db.dao.update(copy)
            Log.i(TAG, "Updated preset profile for $cloneId to $presetNameOrId")
        } else {
            db.dao.insert(copy)
            Log.i(TAG, "Applied preset profile for $cloneId as $presetNameOrId")
        }
        return copy
    }

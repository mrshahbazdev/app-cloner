...
public class VirtualPackageInstaller {
    ...
    private boolean checkAbiCompatibility(PackageInfo info) {
        ...
        String nativeAbi = nativeDir.getName();

        for (String abi : supportedAbis) {
            if (abi.equals(nativeAbi)) return true;
            // arm64-v8a devices can run armeabi-v7a code
            if ("arm64-v8a".equals(abi) && "armeabi-v7a".equals(nativeAbi)) return true;
        }

        Log.w(TAG, "ABI mismatch: app=" + nativeAbi + " device=" + Arrays.toString(supportedAbis));
        return false;
    }
    ...
    private VirtualPackageInfo buildVirtualPackageInfo(
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
            versionCode = sourceInfo.longVersionCode;
        } else {
